...
module.exports = {
  ...
  getCertificateInfo: function (cert, cb) {
    var infoObject = {},
  	...
  	var openssl = spawn('openssl', ['x509', '-noout', '-issuer', '-subject', '-fingerprint', '-dates', '-nameopt', 'RFC2253']);
    ...
    openssl.on('close', function (code) {
      ...
      infoObject.fingerprint = md5(cert).toUpperCase();
      // Issuer
      infoObject.issuer = {};
      // Split by "," separator
      ...
      //Fingerprint
      infoObject.fingerprint = lineArray[2].split('=')[1].replace(/\:/g, '');

      // Dates
      infoObject.validFrom = new Date(lineArray[3].split('=')[1]);
      infoObject.validTo = new Date(lineArray[4].split('=')[1]);
  ...
  getCertificateRequestHash: function (csr, cb) {
   ...
   /**
   * Returns a MD5 hash of a given input key
   * @param  {String|Buffer} key       Input key
   * @param  {Function} cb              Callback
   * @return {Error} err, {String} hash Error and key hash
   */
  getPrivateKeyHash: function (key, cb) {
    var err,
        stdout = "",
        stderr = "";

    var openssl = spawn('openssl', ['rsa', '-noout', '-modulus']);
    var md5 = spawn('openssl', ['md5']);
